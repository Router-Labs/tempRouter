...
**One-liner:** An MPP-paid private LLM where the Tempo **402 session challenge HMAC-binds the live Phala Intel TDX quote digest**, and the paying agent runs a **real Intel DCAP verification** of that quote before signing the first voucher — *pay only what you can prove is private*.
...
> 4. **Payment binding** — the verified `sha256(tdxQuote)` equals the `externalId` the server HMAC-signed into the 402 challenge, so the voucher settles against *one named quote*.
...
- The verify-before-pay decision lives in **our agent**, enforced as ordinary control flow (not an mppx protocol hook — mppx 0.7.0 has none). The server *additionally* refuses to settle vouchers whose `externalId` ≠ the live quote digest, so the **money-not-moving decision is made on BOTH sides** (see §3 step 8).
- The reused crypto + TEE stack is **chain-agnostic by design** — that is a strength (real, audited-in-prod), not the contribution. The contribution is the **HMAC binding of the attestation digest into the MPP session challenge**.
...
| **No MPP novelty** | Bind quote INTO settlement, server-side | Server sets challenge `externalId = sha256(tdxQuote)` (live quote it served) and **refuses to honor any voucher whose bound `externalId` ≠ live quote digest** → tears down channel, no `Sse.serve`, zero transfer. This is new MPP-rail behavior. |
...
| **Proof↔payment seam / report_data mismatch** | Bind digest into HMAC'd `externalId`; pin exact byte formula | Same as MPP-novelty fix. Verifier pins the **`sha512("app-data:"‖sha256(teePub‖enclavePub))`** bytes from `verify-attestation.mjs:48` — the one that passes live. |
...
1. **Agent POSTs** `/v1/chat/completions/stream` with **no credential**. Server replies **402** (`mppx.session({ amount: pricePerUnit, unitType:'response-chunk', description:'Private inference in Phala Intel TDX', externalId })`). The `externalId` is an **attestation commitment**: `sha256(tdxQuote)` of the live quote the server just fetched from `TEE_ENDPOINT/attestation`. mppx HMACs `externalId` with the server `secretKey`.
2. **Agent parses** the 402 (`ChallengedSessionState`), extracts `externalId`, then **`GET /tee/attestation`** on tempRouter (blind passthrough to the real Phala `GET /attestation`) → `{ teeType, teePublicKey, teePublicKeySha256, reportDataHex, tdxQuote }`. For the *paid* path it also triggers a real `/tee/process` attestation envelope (`encryptionProof`) so it has the ed25519 sig + `tdxQuoteHash` to check.
3. **Agent VERIFIES** (`verifyAttestation.ts`, ported verbatim from `verify-attestation.mjs`), fail-closed:
   ...
   - (d) **Payment binding**: `sha256(JSON.stringify(tdxQuote)) === ep.tdxQuoteHash === challenge.externalId`.
...
- `src/server.ts` — `GET /tee/attestation` + `GET /tee/public-key` blind passthrough; set session `externalId = sha256(tdxQuote)`; server-side settlement gate (step 8); never read private-route plaintext. Mount mppx `discovery()` → `GET /openapi.json`; serve `GET /llms.txt`.
- `src/config.ts` — `teeEndpoint`, `mode` resolver (`tdx-live|stub|down`).
- `public/llms.txt` + mppx `discovery()` config — MPP service-discovery surface (`/openapi.json` + `/llms.txt`) so tempRouter is listable on mpp.dev/services + MPPScan.
...
| **T4** | `server.ts` — `GET /tee/attestation`+`/public-key` passthrough; session `externalId=sha256(tdxQuote)`; **server settlement gate** (reject voucher if bound `externalId`≠ live quote digest → no `Sse.serve`). | `server.ts` | Curl `/tee/attestation` mirrors enclave; forged-externalId voucher → channel torn down, zero stream (server log shows refusal) | **L** | T1, T3 |
| **T5** | **MPP discovery**: mount mppx `discovery()` → `GET /openapi.json` with `x-service-info` (`categories`, `docs.homepage`, `docs.llms`) + per-op `x-payment-info.offers` ({amount, pathUSD, intent:'session', method:'tempo'}) + `402` response; serve `GET /llms.txt`. Draft the mpp.dev/services PR + MPPScan submission. | `server.ts`, `public/llms.txt` | `/openapi.json` validates as MPP discovery doc (x-service-info + x-payment-info present, 402 declared); `/llms.txt` 200s; PR drafted (not opened w/o ok) | M | T4 |
...
**[1:15–2:30] Run 2 — the wow.** Point at the real Phala CVM. Agent prints `TDX verified: INTEL-TDX-PHALA, DCAP ok, key bound, sig ok, quote digest == challenge externalId`. **Split screen:** left = `tcpdump`/proxy log showing the request body is **ciphertext** (not plaintext `messages`); right = tokens streaming in *while the on-chain pathUSD balance ticks* on the explorer. Close channel → ~2 txs. "Money moved only because privacy was provable, and the bytes on the wire were ciphertext — proven, not promised."

**[2:30–3:00] The MPP point + honest scope.** "What's new on Tempo: the server HMAC-binds `sha256(tdxQuote)` into the 402 `externalId`, and refuses to settle a voucher against a different quote — payment is bound to a *named attested enclave*. We verify the Intel cert chain + key binding + enclave signature; we do **not** pin code measurement — that's next."
...
2. **"Show me the one byte of NEW MPP behavior — your gate is just a client if-statement."**
   → The server sets the 402 `externalId = sha256(tdxQuote)` (HMAC'd with `secretKey`) and **refuses to settle any voucher bound to a different quote digest**, so the money-not-moving decision in Run 1 is made by the *server's* refusal, not the client — challenge-`externalId`-as-attestation-commitment + settlement-conditioned-on-it is the new MPP-rail artifact.
...
5. **"What ties a signed Tempo voucher to the specific attested enclave — your report_data is the wrong value anyway?"**
   → The voucher carries the HMAC'd `externalId = sha256(tdxQuote)` of the exact quote the agent DCAP-verified, so flipping the enclave invalidates the challenge HMAC (not just a client check); and we pin the *correct* prod-passing byte formula `sha512("app-data:"‖sha256(teePub‖enclavePub))`, scoping the claim to "DCAP cert-chain + key-binding + enclave-signature + voucher-bound-to-quote — no code-measurement pin," exactly as printed on the slide.
